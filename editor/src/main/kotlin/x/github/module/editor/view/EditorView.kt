...
open class EditorView @JvmOverloads constructor(
    ...
    protected var mergeTextChanges = emptyList<TextChange>()
    ...
    fun setSearchResult(matches: MutableList<Range>?) {
        searchResults.clear()
    ...
    fun getSearchResult() = searchResults
    ...
    fun scrollToMatchRange(range: Range) {        
        val dx = getStartSpacing() + getLineWidthAdvance(range.startLine, 1, range.startColumn)
    ...
    // #endregion
    @JvmName("setActionModeCallback")
    fun setActionCallback(actionCallback: ActionMode.Callback) {
    ...
    val cx: Int get() = cursor.getX()
    
    val cy: Int get() = cursor.getY()
    ...
    // Text Cursor
    protected inner class TextCursor(position: Position? = null) : Position() {
    ...
    // Text Selector
    protected inner class TextSelector(range: Range? = null) : Range() {
    ...
    protected var isSingleTapUp = false
